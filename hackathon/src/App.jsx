...
import {  getAccountById, getAccessToken, login,getAgreement, getPrediction } from './API/api';
...
const loadPrediction = async (
  ...
  try {
    ...
    const payload = {
      ...
      discount:
        agreement.parameters.discount,

      supportTickets:
        agreement.parameters.supportTickets,

      inactiveDays:
        agreement.parameters.inactiveDays,

      clauseRisk:
        agreement.parameters.clauseRisk,

      quoteRevisions:
        agreement.parameters.quoteRevisions
    };
  ...
  useEffect(() => {
    const loadAccountDetails = async () => {
      ...
      try {
        ...
        console.log('Fetching account details for', ag.accountId);
          console.log('Fetching account details for', ag);
        const acc = await getAccountById(ag.accountId);
        setAccountDetails(acc || null);
console.log('Loaded account details for', ag.accountId, acc);
